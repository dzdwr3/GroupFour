...
public class BasePage {
	 ...
	 * @param locator
	 */
	public void switchToIfream(Locator locator)
	{
		WebElement e = findElement(driver, locator);
	 ...
	 * 这个方法用来将隐藏的元素显示
	 */
	public void makeDisplayToBlock(Locator locator)
	{
		WebElement e = findElement(driver, locator);
		JavascriptExecutor js=(JavascriptExecutor)driver;
	...
	protected void clearInputText(Locator locator) throws Exception
	{
		click(locator);
	...
	/**
	 * @author Zither
	 * 这个方法用来方便操作需要滚动滚动条的才能找到的元素
	 */
	
	protected void pageDown() throws Exception {
		Actions action = new Actions(driver); 
		log.info("Enter pageDown");
		action.sendKeys(Keys.PAGE_DOWN).perform();
		Thread.sleep(2000);
		log.info("Enter pageDown sucess");
	}
	...
	public WebElement getElement(WebDriver driver, Locator locator)
			throws IOException {
		log.debug("1"+locator.getElement());
		locator = getLocator(locator.getElement());
		log.debug("2"+locator.getElement());
		WebElement e;
